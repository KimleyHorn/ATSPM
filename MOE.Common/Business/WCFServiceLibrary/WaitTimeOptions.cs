...
namespace MOE.Common.Business.WCFServiceLibrary
{
    ...
    public class WaitTimeOptions : MetricOptions
    {
        ...
        public override List<string> CreateMetric()
        {
            ...
            var eventLogs = new ControllerEventLogs(SignalID, StartDate, EndDate, new List<int> { PHASE_BEGIN_GREEN, PHASE_END_RED_CLEARANCE, PHASE_CALL_REGISTERED, PHASE_CALL_DROPPED });
            foreach (var approach in signal.Approaches.OrderBy(x => x.ProtectedPhaseNumber))
            {
                var phaseInfo = analysisPhaseCollection.Items.FirstOrDefault(x => x.PhaseNumber == approach.ProtectedPhaseNumber);
                CreateChart(approach, eventLogs, signal, phaseInfo, analysisPhaseCollection.Plans);
            }
            return ReturnList;
        }

        public void CreateChart(Approach approach, ControllerEventLogs eventLogs, Signal signal, AnalysisPhase phaseInfo, List<PlanSplitMonitor> plans)
        {
            var phaseEvents = eventLogs.Events.Where(x => x.EventParam == approach.ProtectedPhaseNumber);

            if (phaseEvents.Any())
            {
                var waitTimeChart = new WaitTimeChart(this, signal, approach, phaseEvents, StartDate, EndDate, phaseInfo, plans);
        ...
        public Series CreateChartSeries(Color color, SeriesChartType seriesChartType, ChartValueType xValueType, AxisType yAxisType, string name)
        {
            return new Series
            {
