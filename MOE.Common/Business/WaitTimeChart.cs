...
namespace MOE.Common.Business
{
    class WaitTimeChart
    {
        ...
        public WaitTimeChart(WaitTimeOptions waitTimeOptions, Signal signal, Approach approach, IEnumerable<Controller_Event_Log> phaseEvents, DateTime startDate, DateTime endDate, AnalysisPhase phaseInfo, List<PlanSplitMonitor> plans)
        {
        ...
        private void SetChartTitle(Chart chart, WaitTimeOptions waitTimeOptions, Signal signal, Approach approach, string detectionTypes)
        {
            chart.Titles.Add(ChartTitleFactory.GetChartName(waitTimeOptions.MetricTypeID));
            chart.Titles.Add(ChartTitleFactory.GetSignalLocationAndDateRange(signal.SignalID, waitTimeOptions.StartDate, waitTimeOptions.EndDate));
        ...
        {
            var waitTimeSeries = _waitTimeOptions.CreateChartSeries(Color.Blue, SeriesChartType.Point, ChartValueType.DateTime, AxisType.Primary, "Wait Time (s)");
            _waitTimeOptions.CreateVolumeSeries(Chart);

            var redList = _phaseEvents.Where(x => x.EventCode == WaitTimeOptions.PHASE_END_RED_CLEARANCE).OrderBy(x => x.Timestamp);
            var greenList = _phaseEvents.Where(x => x.EventCode == WaitTimeOptions.PHASE_BEGIN_GREEN).OrderBy(x => x.Timestamp);
            var orderedPhaseRegisterList = _phaseEvents.Where(x => x.EventCode == WaitTimeOptions.PHASE_CALL_REGISTERED || x.EventCode == WaitTimeOptions.PHASE_CALL_DROPPED);
            ...
            try
            {
                foreach (var red in redList)
                {
                    ...
                    var green = greenList.Where(x => x.Timestamp > red.Timestamp).OrderBy(x => x.Timestamp).FirstOrDefault();
                    if (green == null)
                        continue;

                    //Find all events between the red and green
                    var phaseCallList = orderedPhaseRegisterList.Where(x => x.Timestamp >= red.Timestamp && x.Timestamp < green.Timestamp).OrderBy(x => x.Timestamp).ToList();
                    ...
                    if (useDroppingAlgorithm && phaseCallList.Any(x => x.EventCode == WaitTimeOptions.PHASE_CALL_DROPPED))
                    {
                        var lastDroppedPhaseCall = phaseCallList.LastOrDefault(x => x.EventCode == WaitTimeOptions.PHASE_CALL_DROPPED);
                        if (lastDroppedPhaseCall != null)
                        {
                            ...
                            //waitTimeTrackerList.Add(new WaitTimeTracker { Time = green.Timestamp, WaitTimeSeconds = (green.Timestamp - nextPhaseCall.Timestamp).TotalSeconds });
                            waitTimeTrackerToFill = new WaitTimeTracker { Time = green.Timestamp, WaitTimeSeconds = (green.Timestamp - nextPhaseCall.Timestamp).TotalSeconds };
                        }
                    }
                    else
                    {
                        ...
                        waitTimeTrackerToFill =  new WaitTimeTracker { Time = green.Timestamp, WaitTimeSeconds = (green.Timestamp - firstPhaseCall.Timestamp).TotalSeconds };
                    }
                    ...
                    if (priorPhase != null)
                    {
                        ...
                        {
                            case 4: //Gap Out
                                GapoutSeries.Points.AddXY(waitTimeTrackerToFill.Time, waitTimeTrackerToFill.WaitTimeSeconds);
                                break;
                            case 5: //Max Out
                                MaxOutSeries.Points.AddXY(waitTimeTrackerToFill.Time, waitTimeTrackerToFill.WaitTimeSeconds);
                                break;
                            case 6: //Force Off
                                ForceOffSeries.Points.AddXY(waitTimeTrackerToFill.Time, waitTimeTrackerToFill.WaitTimeSeconds);
                                break;
                            case 0:
                                UnknownSeries.Points.AddXY(waitTimeTrackerToFill.Time, waitTimeTrackerToFill.WaitTimeSeconds);
                                break;
                        }
                    }
                }
            ...
            try
            {
                ...
                //todo: customizable bin size
                for(var lowerTimeLimit = _startDate; lowerTimeLimit < _endDate; lowerTimeLimit = lowerTimeLimit.AddMinutes(15))
                {
                    ...
                    {
                        //AverageSeries.Points.AddXY(upperTimeLimit, 0);
                    }
                }

                //Toss anything longer than 10 minutes - usually a bad value as a result of missing data
                //waitTimeTrackerList.RemoveAll(x => x.WaitTimeSeconds > 600);

                //foreach (var row in waitTimeTrackerList)
                //{
                //    waitTimeSeries.Points.AddXY(row.Time, row.WaitTimeSeconds);
                //}

                //Round to the nearest minute
                Chart.ChartAreas[0].AxisY.Maximum = Math.Ceiling(waitTimeTrackerList.Max(x => x.WaitTimeSeconds) / 60d) * 60;
                //Chart.Series.Add(waitTimeSeries);
                ...
                if (_waitTimeOptions.ShowPlanStripes)
                {
                    ...
                    //var analysisPhaseCollection = new AnalysisPhaseCollection(_signal.SignalID, _startDate, _endDate);
                    //SetSimplePlanStrips(analysisPhaseCollection.Plans, Chart, _startDate);
                }
            }
            catch(Exception ex)
            {
        ...
        private void SetSimplePlanStrips(List<PlanSplitMonitor> plans, Chart chart, DateTime graphStartDate, List<WaitTimeTracker> waitTimeTrackerList)
        {
            ...
            foreach (var plan in plans)
            {
                ...
                if (waitTimeTrackerList.Any())
                {
                    var waitTimeSubset = waitTimeTrackerList.Where(x => x.Time > plan.StartTime && x.Time < plan.EndTime);
                    if (waitTimeSubset.Any())
                    {
                        ...
                        var avgLabel = ChartTitleFactory.GetCustomLabelForTitle($"Avg Wait: {avgWaitTime.ToString("F1")} s", plan.StartTime.ToOADate(), plan.EndTime.ToOADate(), 5, Color.Black);
                        chart.ChartAreas["ChartArea1"].AxisX2.CustomLabels.Add(avgLabel);

                        var maxLabel = ChartTitleFactory.GetCustomLabelForTitle($"Max Wait: {maxWaitTime.ToString("F1")} s", plan.StartTime.ToOADate(), plan.EndTime.ToOADate(), 4, Color.Black);
        ...
        private void SetPlanStrips(List<PlanPcd> plans, Chart chart)
        {
            var backGroundColor = 1;
            foreach (var plan in plans)
            {
                ...
                //Set the stripline properties
                stripline.IntervalOffset = (plan.StartTime - _startDate).TotalHours;
                stripline.IntervalOffsetType = DateTimeIntervalType.Hours;
                stripline.Interval = 1;
